...
If GBIF API access is blocked (e.g. proxy restrictions), the fetch script falls back to a bundled GBIF example archive from `rgbif` for pipeline smoke testing. Disable with `GBIF_ALLOW_EXAMPLE_FALLBACK=false`.
